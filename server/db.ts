...
/** Admin: get all confirmed matches with classStatus = 'cancelled' (enriched) */
export async function getCancelledConfirmedMatches() {
  ...
  const rows = await db
    .select({
      ...
      studentProfileId: confirmedMatches.studentProfileId,
      tutorName: confirmedMatches.tutorName,
      tutorEmail: confirmedMatches.tutorEmail,
      tutorPhone: confirmedMatches.tutorPhone,
      studentName: confirmedMatches.studentName,
      studentEmail: confirmedMatches.studentEmail,
      studentPhone: confirmedMatches.studentPhone,
      studentArea: confirmedMatches.studentArea,
      studentGrade: confirmedMatches.studentGrade,
      studentSubjects: confirmedMatches.studentSubjects,
      paymentAmount: confirmedMatches.paymentAmount,
      classStatus: confirmedMatches.classStatus,
      cancellationRequestedBy: confirmedMatches.cancellationRequestedBy,
      cancellationNote: confirmedMatches.cancellationNote,
...
/** Admin: Smart Pairs — student+tutor pairs within 10km, matching gender preference & class requirements */
export async function getSmartPairs(radiusKm = 10) {
  const db = await getDb();
  if (!db) return [];

  const allTutors = await db.select().from(tutorProfiles).where(eq(tutorProfiles.status, 'approved'));
  const allStudents = await db.select().from(studentProfiles).where(eq(studentProfiles.isActive, 'yes'));

  // Get all active (non-cancelled) confirmed match pairs to exclude already-matched pairs
  const activeMatchRows = await db.select({
    tutorProfileId: confirmedMatches.tutorProfileId,
    studentProfileId: confirmedMatches.studentProfileId,
  }).from(confirmedMatches).where(ne(confirmedMatches.classStatus, 'cancelled'));
  const activeMatchSet = new Set(activeMatchRows.map(r => `${r.tutorProfileId}:${r.studentProfileId}`));

  const pairs: Array<{
    tutorProfileId: number; studentProfileId: number;
    tutorName: string; tutorPhone: string | null; tutorEmail: string;
    tutorArea: string | null; tutorSubjects: string; tutorQualification: string;
    tutorExperience: string; tutorGender: string | null;
    studentName: string; studentPhone: string; studentEmail: string;
    studentArea: string | null; studentGrade: string; studentSubjects: string;
    tutorGenderPreference: string | null; distanceKm: number;
  }> = [];

  for (const tutor of allTutors) {
    if (!tutor.latitude || !tutor.longitude) continue;
    const tLat = parseFloat(String(tutor.latitude));
    const tLng = parseFloat(String(tutor.longitude));
    if (isNaN(tLat) || isNaN(tLng)) continue;

    for (const student of allStudents) {
      if (!student.latitude || !student.longitude) continue;
      const sLat = parseFloat(String(student.latitude));
      const sLng = parseFloat(String(student.longitude));
      if (isNaN(sLat) || isNaN(sLng)) continue;

      if (activeMatchSet.has(`${tutor.id}:${student.id}`)) continue;

      const dist = haversineKm(tLat, tLng, sLat, sLng);
      if (dist > radiusKm) continue;

      // Check gender preference
      const genderPref = student.tutorGenderPreference;
      if (genderPref && genderPref !== 'no_preference' && tutor.gender && tutor.gender !== genderPref) continue;

      // Check subject overlap (simple substring match)
      const tutorSubjectsLower = (tutor.subjects ?? '').toLowerCase();
      const studentSubjectList = (student.subjects ?? '').toLowerCase().split(/[,;/]+/).map(s => s.trim()).filter(Boolean);
      const hasSubjectMatch = studentSubjectList.length === 0 ||
        studentSubjectList.some(sub => tutorSubjectsLower.includes(sub) || sub.split(' ').some(w => w.length > 2 && tutorSubjectsLower.includes(w)));

      if (!hasSubjectMatch) continue;

      pairs.push({
        tutorProfileId: tutor.id, studentProfileId: student.id,
        tutorName: tutor.name, tutorPhone: tutor.phone, tutorEmail: tutor.email,
        tutorArea: tutor.area ?? null, tutorSubjects: tutor.subjects,
        tutorQualification: tutor.qualification, tutorExperience: tutor.experience,
        tutorGender: tutor.gender ?? null,
        studentName: student.name, studentPhone: student.phone, studentEmail: student.email,
        studentArea: student.area ?? null, studentGrade: student.grade, studentSubjects: student.subjects,
        tutorGenderPreference: student.tutorGenderPreference ?? null,
